...
mm=mtrySelJack(Y=cordist,X=env,covariates=latlon.gcd,nreps=31, prop.positive.cutoff=0.5,top.pcs=tokeep,ncores=4)
```
...
mm$goodvars
```
Dropped variables that are strongly correlated (|r| >= 0.7, set by `report.cor`) with retained ones. The `mtry` test treats these as proxies of the retained variables, which is usually right; but two variables that are both real drivers and correlated can lose to each other the same way, so it is worth a look whether any of these is plausibly a driver in its own right:
```{r}
mm$dropped.pairs
```
...
\
Both environments are scored in one run: `newX` is given as a named list, and every jackknife replicate scores both of them before its models are discarded. This matters for the genetic offset computed further below: the offset is the difference between the two predictions, so if each were made by its own `ordinationJackknife()` run, that difference would carry the sampling noise of two independent sets of forests on top of the environmental change we actually want to measure. Scoring both through the same replicates removes it, halves the running time, and, because no replicate's models are kept once they have been used, keeps the memory flat. `extra` (and `extra.npred`) can take one value per environment, so the future can be allowed to reach further beyond the modelled range than the present.\
\
`ncores` runs replicates in parallel. Each worker needs its own memory (roughly 0.8 GB here), so set it to what your RAM allows, not just to the number of cores; a worker that runs out of memory is killed and the function stops saying so.\
\

```{r results='hide', message=FALSE, warning=FALSE}
res=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],covariates=latlon.gcd,nreps=25,top.pcs=tokeep,
                        newX=list(present=envc,future=envf),extra=c(0.1,0.2),ncores=4)
oj=res$present   # present-day
ojf=res$future   # future (2041-2060)
# save(mm,gf,oj,ojf,file="models_wolf.RData")
```
To compare several future scenarios, add them to the same list (for example `newX=list(present=envc,ssp245=envf245,ssp585=envf585)`): every scenario is then scored through the same replicates, at hardly any extra cost.\
\
To score scenarios that are not yet at hand, run with `keep.models=TRUE` and score them later with `ojPredict(res, newX=..., extra=...)`, through the same replicates. The kept models take about 1.2 GB here.\
\
...
\
We have already generated future predictions (`ojf`), so let's compare them to the present-day (`oj`). Both were scored through the same jackknife replicates, so what follows is the environmental difference alone, with no contribution from two models having been fitted on different random draws. The function `gen_offset_oj()` requires four arguments: the two prediction objects for present-day and future, and two sets of spatial coordinates, for present-day and future predictions (for aligning the two).\
