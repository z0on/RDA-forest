...
logd.ord=capscale(log(distGCD+1)~1)
pcs.logd=scores(logd.ord,scaling=1,display="sites",choices=c(1:2))

# constructing and plotting partial ordination
ord1=capscale(cordist~1+Condition(as.matrix(pcs.logd)))
...
mm=mtrySelJack(Y=cordist,X=env,covariates=pcs.logd,nreps=30, prop.positive.cutoff=0.5, top.pcs=tokeep)
```
...
We are only using the variables we selected above. Note that we are still using `pcs.logd` as a covariate, to account for IBD.\ `newX` here is a dataframe of environmental conditions across the landscape where adaptation is to be predicted. This data table must contain all the predictors that the model uses.\
\
...
```{r results='hide', message=FALSE, warning=FALSE}
# present-day
oj=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],newX=envc,covariates=pcs.logd,nreps=20,top.pcs=tokeep,extra=0.1)
# future
ojf=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],newX=envf,covariates=pcs.logd,nreps=20,top.pcs=tokeep,extra=0.1)
...
\
```{r results='hide', message=FALSE, warning=FALSE}
oj0=ordinationJackknife(Y=cordist0,X=env[-i,mm$goodvars],newX=envc,covariates=pcs.logd[-i,],nreps=20,top.pcs=tokeep,extra=0.1)
```
...
ord0=capscale(cordist0~1+Condition(as.matrix(pcs.logd[-i,])))
