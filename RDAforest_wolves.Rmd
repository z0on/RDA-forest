...
\
This procedure can be made less strict (i.e. retain more predictors) by setting `prop.positive`, the proportion of replicates in which importance has to increase under higher `mtry`, to something less than 0.333 (the default), like we do here. Another way to achieve less strict selection is to manually specify  `mintry = 3` and `maxtry = 6` (these would be used by default for situations with less than 12 predictors, like we have here).\ 

Note: this step will take a bit

```{r results='hide', message=FALSE, warning=FALSE}
mm=mtrySelJack(Y=cordist,X=env,covariates=latlon.utm,nreps=20, top.pcs=tokeep,prop.positive=0.25)
```
...
Bar chart of proportion of positive change in response to higher `mtry`. Good variables would be the ones above the red line (change `yintersept` setting here to match `prop.positive` in the call to `mtrySelJack`) :
```{r}
...
```{r results='hide', message=FALSE, warning=FALSE}
oj=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],newX=predictors,covariates=latlon.utm,nreps=20,top.pcs=tokeep)
```
...
Subsetting data to the range where predictions were allowed:
```{r}
...
<!-- ALTERNATIVE to map plotting, with no jackknifing/averaging -->
<!-- (use this chunk instead of the previous one) -->

```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
topPCs=data.frame(scores(ord1,scaling=1,display="sites",choices=c(1:tokeep)))
p.rf=predict_rf(topPCs,env[,mm$goodvars],newX=predictors,ntree=1500,mtry=2)
p.gf=predict_gf(topPCs,env[,mm$goodvars],newX=predictors,ntree=1500,mtry=2)
goods=p.rf$goodrows
ras2=predictors[which(goods),]
xy2=predictors[goods,c("lon","lat")]
rfpreds=p.rf$preds
turnovers=p.gf$preds
bests=mm$goodvars
```
...
That this map nicely separates most ecotypes except it is having some trouble with "Arctic" - "High Arctic" ones. Note that this map suggests that "West forest" and "Atlantic forest" ecotypes are essentially the same, adaptation-wise.
...
For this dataset the two maps look very similar, really impossible to say if one is in any way more accurate than another. 
