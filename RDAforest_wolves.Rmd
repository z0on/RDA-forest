...
# (for situations when you need to keep spatial coordinates among predictors and then use them for inferring adaptation on landscape)
...
Variables below red line are correlated with r > 0.9! Still let's keep all of them.
...
The colored plots are turnovers of individual PCs (from MDS1 to MDS8), and the black-and-white plots are their averages. Colored plots don't have predictor labels below x-axes, but their order corresponds to the black and white plots, which are labeled. You can see three things: \
  - which PC is affected by which variable. For example, PC1 (`MDS1`, red line on colored plots) is strongly affected by `MeanT_WarmQ` (and a couple other variables). \
...
mm=mtrySelJack(Y=cordist,X=env,covariates=latlon.gcd,nreps=50, top.pcs=tokeep)
```
...
ggplot(mm$prop.positive,aes(var,prop.positive))+
  ...
  coord_flip()+
  geom_hline(yintercept=0.333,col="red")
```
...
```{r results='hide', message=FALSE, warning=FALSE}
# present-day
oj=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],newX=envc,covariates=latlon.gcd,nreps=50,top.pcs=tokeep,extra=0.1)
# future
ojf=ordinationJackknife(Y=cordist,X=env[,mm$goodvars],newX=envf,covariates=latlon.gcd,nreps=50,top.pcs=tokeep,extra=0.2)
...
\
We have already generated future predictions (`ojf`), so let's compare them to the present-day (`oj`). The function `gen_offset_oj()` requires four arguments: `ordinationJaccknife()` objects for present-day and future, and spatial coordinates of the present-day and future predictions (for aligning the two).\
...
# rescaling factor: 50% quantile of present-day adaptation disparity
sc=adapt_scale(oj$predictions.direct)[1]
...
ggplot()+geom_sf(data=landscape)+
  ...
  geom_point(data=data.frame(rbind(park)),aes(X1,X2),pch=0)

```

Our park is the little square in the middle. There is a clearly preferred area for our assisted gene flow provenance southeast of the park, just above the USA-Canada border (shaped a bit like a goat facing West).
...
\
```{r message=FALSE, warning=FALSE}
# let's pick an "unknown wolf" from our 94. This one is Atlantic Forest ecotype. Feel free to try a Britich Columbia (i=53) or Boreal forest (i=40), or any other really.
...
We will now build the landscape model without the unknown wolf (will take a while):\
\
```{r results='hide', message=FALSE, warning=FALSE}
tokeep=15
oj0=ordinationJackknife(Y=cordist0,X=env[-i,mm$goodvars],newX=envc,covariates=latlon.gcd[-i,],nreps=50,top.pcs=tokeep,extra=0.1)
```
...
home=env_mismatch(X=wi,Y=oj0,sy=envc[,1:2],sc=1)
...
ggplot()+geom_sf(data=landscape)+
  ...
  theme_minimal()+
  geom_raster(data=home,aes(x=x,y=y,fill=env.mismatch))+scale_fill_viridis(option="magma",direction = -1)+
  geom_point(data=data.frame(latlon[i,]),aes(lon,lat),pch=0)

```

The model correctly identified where the wolf came from, just from its genotype - the square on the map (actual wolf origin, unknown to the model) lies wihtin the lightest color area. Interestingly, this particular wolf seems to have a rather narrow optimal spatial range.
