...
The RDA-forest functions come in the form of an R package, `RDAforest_2.2.0.tar.gz`. To install it, run this in Rstudio
```R
install.packages("/path/to/downloaded/file/RDAforest_2.2.0.tar.gz")
library(RDAforest)
```
...
- `ordinationJackknife` : Runs *gradientForest* on *nreps* jackknife replicates, each time rebuilding the ordination withholding a fraction of datapoints (default 0.2). as of version 2.2.0, forms predictions of two kinds - turnover curves and straight-up random forest, to be used for adaptation plotting (`plot_adaptation` function) - for the supplied *newX* data, averaging them over replicates. Makes sure no predictions are made beyond the numeric range of predictors used to fit the model, but can allow a bit of extension specified as fraction of the original range (option *extra*).
- `plot_adaptation` : Plots first two or three PCs of the supplied matrix ( *[result of predict_rf]$preds* or *[result of ordinationJackknife]$predictions.direct*), then plots geographical map of adaptation colored according to these PCs. Can color points by their continuous values along PCs, or by cluster they fall into (with `nclust` argument). Clustering is done using function *cluster::clara*. Starting with version 2.1.1 can use turnover curves (*[result of predict_gf]$preds* or *[result of ordinationJackknife]$predictions.turnover*) for clustering (option *clustering.guide*) and merge clusters if they are too similar accoring to *predict_rf* or *[result of ordinationJackknife]$predictions.direct*. This can generate more robust clustering than clustering random forest predictions straight up. Cluster merging is controlled by *cluster.merge.threshold* parameter, which is specified as the fraction of the maximal observed between-cluster distance (default 0.333).
...
- `predict_rf` : (To be used directly on *Y* data, without ordination or jackknifing) Predicts *Y* values based on `extendedForest::randomForest`. These values are used for plotting adaptation (see `plot_adaptation`). Handles modeled and predicted ranges like *ordinationJackknife*. 
- `predict_gf` : (To be used directly on *Y* data, without ordination or jackknifing) Generates turnover curves with *gradientForest*. These should **not** be used plotting adaptation but are very good for clustering points into adaptive neighborhoods (see *plot_adaptation*). Handles modeled and predicted ranges like *ordinationJackknife*.
...
- `latlon2UTM`, `epsg.maker` - accessory functions for *plot_nice_map*.
