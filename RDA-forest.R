...
# covars: covariates (to regress out of the ordination)
...
# IBS:    square matrix of genetic distances. In this case, this is identity-by-state matrix made by ANGSD,
...
mm=mtrySelJack(Y=IBS,X=cbind(latlon,env),covariates=covars, nreps=25, top.pcs=15)
...
# histogram: are there clear winners? (mode near 1.0)
...
sb.env=ordinationJackknife(Y=IBS,X=env[,mm$goodvars[!(mm$goodvars %in% c("lat","lon"))]],nreps=25,covariates=covars,top.pcs=15)
# just lat, lon
