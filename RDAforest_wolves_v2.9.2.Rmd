...
\
Since both predictions come from the same model, we fit the jackknife replicates once with `ojFit()` and then score each environment through them with `ojPredict()`. This matters for the genetic offset computed further below: the offset is the difference between the two predictions, so if each were made by its own `ordinationJackknife()` run, that difference would carry the sampling noise of two independent sets of forests on top of the environmental change we actually want to measure. Fitting once removes it, and halves the running time into the bargain. `ojPredict()` takes its own `extra` (and `extra.npred`), so the two environments can still be allowed to reach different distances beyond the modelled range.\
\

```{r results='hide', message=FALSE, warning=FALSE}
# fit the jackknife replicates ONCE ...
ojfit=ojFit(Y=cordist,X=env[,mm$goodvars],covariates=latlon.gcd,nreps=25,top.pcs=tokeep)
# ... then score both environments through the same ensemble
# present-day
oj=ojPredict(ojfit,newX=envc,extra=0.1)
# future
ojf=ojPredict(ojfit,newX=envf,extra=0.2)
# save(mm,gf,ojfit,oj,ojf,file="models_wolf_v4.2.RData")

```
`ojfit` holds every fitted forest, so it is a large object --- roughly 90 Mb per retained principal component with 25 replicates of 1500 trees, so about 1.3 GB at the package defaults. Drop it with `rm(ojfit)` once the predictions are made. The same thing can be had from `ordinationJackknife()`, which keeps its ensemble in `$fit` by default: `oj=ordinationJackknife(...,newX=envc,extra=0.1)` and then `ojf=ojPredict(oj$fit,newX=envf,extra=0.2)`. If only one environment is ever to be scored, `keep.models=FALSE` releases each replicate as soon as it has been used and keeps the memory flat.\
\
